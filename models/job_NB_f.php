...
class Job_NB_f extends Db
{
    ...
    static function getJob_tuyen_gap($number)
    {
        ...
        INNER JOIN cty ON job_xkld_nb.id_cty = cty.id_cty
    ...
    {
        $sql = self::$connection->prepare("UPDATE job_xkld_nb SET view = view + 1 where id_job = $id");
        $sql->execute();
    ...
    static function getAllJob_andCreatePagination($page, $resultsPerPage)
    {
        ...
        $sql = self::$connection->prepare("SELECT * FROM job_xkld_nb INNER JOIN cty ON job_xkld_nb.id_cty = cty.id_cty where job_xkld_nb.id_trangthai = 1 order by job_xkld_nb.updated_at desc LIMIT $firstLink, $resultsPerPage;");
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $items;
