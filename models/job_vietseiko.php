...
class Vietseiko_f extends Db
{
    ...
    public function timeAgo($time_ago)
    {
     ...
     {
         $sql = self::$connection->prepare("SELECT * FROM job_vietseiko
         INNER JOIN cty ON job_vietseiko.id_cty = cty.id_cty
    ...
    static function getAllJob_andCreatePagination($page, $resultsPerPage)
    {
        ...
        $sql = self::$connection->prepare("SELECT * FROM job_vietseiko INNER JOIN cty ON job_vietseiko.id_cty = cty.id_cty where job_vietseiko.id_trangthai = 1 order by job_vietseiko.updated_at desc LIMIT $firstLink, $resultsPerPage;");
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $items;
    ...
    {
        $sql = self::$connection->prepare("UPDATE job_vietseiko SET view = view + 1 where id_job = $id");
        $sql->execute();
