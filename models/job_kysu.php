...
class Job_Kysu extends Db {
    ...
    /**
     * LẤY RA DỮ LIỆU BẢNG job_kysu
     */
    static function getAllJob_KySuNB() {
    ...
    //Lấy danh sách tất cả job_kysu và Phân trang:
    static function getAllJobKySu_NB_andCreatePagination($page, $resultsPerPage) {
     ...
     */
    static function getLatestJob_KySuNB($number_of_records) {
     ...
     * Lấy chi tiết job_NB:
     */
    function getJobKySu_Detail($id)
    {
        $sql = self::$connection->prepare("SELECT * FROM job_kysunb WHERE name_kysu = ?");
        $sql->bind_param("i", $id);
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
    ...
    function getTypeManuID($id){
        $sql = self::$connection->prepare("SELECT `id_nganhnghe_NB` FROM `job_kysunb` WHERE job_nhatban.id_NB = ?");
        $sql->bind_param("i",$id);
     ...
     */
    static function getJobKySu_NB_ByID($id_nb){
    ...
    static function searchJobKySu_NB($keyword) {
        $sql = self::$connection->prepare("SELECT * FROM job_kysunb WHERE name like '%$keyword%'");
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $items; //return an array.
