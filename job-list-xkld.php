...
$job_tyen_gap = $job_nb::getJob_tuyen_gap($resultsPerPage);
if (isset($_GET['page'])) {
    $page = $_GET['page'];
} else {
...
if (
    ...
    $kq = $job_nb::searchJob($id_nganhnghe, $id_hinhthuc, $id_kinhnghiem, $id_gioitinh, $diachi, $search);
    $totalPages = ceil(floatval(count($kq)) / floatval($resultsPerPage));
    // phân trang
    $search_phantrang = $job_nb::searchJob_and_Phantrang($id_nganhnghe, $id_hinhthuc, $id_kinhnghiem, $id_gioitinh, $diachi, $search, $page, $resultsPerPage);
    $allJob = $search_phantrang;
    $totalJob = count($kq);
} else {
    $allJob = $job_nb::getAllJob_andCreatePagination($page, $resultsPerPage);
    $totalJob = count($job_nb::getAllJob());
...
<meta name="description" content="Việc làm Xuất khẩu lao động Nhật Bản" />
